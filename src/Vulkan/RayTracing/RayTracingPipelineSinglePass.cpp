...
namespace Vulkan::RayTracing
{
	...
	RayTracingPipelineSinglePass::RayTracingPipelineSinglePass(
		...
		const Assets::Scene &scene) : RayTracingPipelineBase(deviceProcedures, swapChain, accelerationStructure, accumulationImageView, outputImageView, uniformBuffers, scene)
	{
		...
		const std::vector<DescriptorBinding> descriptorBindings =
			{
				...
				{7, 1, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_INTERSECTION_BIT_KHR},
		...
		for (uint32_t i = 0; i != swapChain.Images().size(); ++i)
		{
			...
			// Volume Image and texture samplers.
