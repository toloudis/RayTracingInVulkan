...
namespace Vulkan
{
	...
	Image::Image(const class Device &device, const VkExtent2D extent, const VkFormat format) : Image(device, extent, format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT)
	{
	}
	...
	Image::Image(
		...
		const VkImageUsageFlags usage) : device_(device),
										 extent_(extent),
										 format_(format),
	...
	Image::Image(Image &&other) noexcept : device_(other.device_),
	...
	VolumeImage::VolumeImage(const class Device &device, const VkExtent3D extent, const VkFormat format) : VolumeImage(device, extent, format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT)
	{
	}
	...
	VolumeImage::VolumeImage(VolumeImage &&other) noexcept : device_(other.device_),
															 extent_(other.extent_),
															 format_(other.format_),
															 imageLayout_(other.imageLayout_),
															 image_(other.image_)
	...
	DeviceMemory VolumeImage::AllocateMemory(const VkMemoryPropertyFlags properties) const
	{
		const auto requirements = GetMemoryRequirements();
		DeviceMemory memory(device_, requirements.size, requirements.memoryTypeBits, 0, properties);
	...
	VkMemoryRequirements VolumeImage::GetMemoryRequirements() const
	{
		VkMemoryRequirements requirements;
		vkGetImageMemoryRequirements(device_.Handle(), image_, &requirements);
		return requirements;
	...
	void VolumeImage::TransitionImageLayout(CommandPool &commandPool, VkImageLayout newLayout)
	{
		SingleTimeCommands::Submit(commandPool, [&](VkCommandBuffer commandBuffer)
								   {
		VkImageMemoryBarrier barrier = {};
	...
	void VolumeImage::CopyFrom(CommandPool &commandPool, const Buffer &buffer)
	{
		SingleTimeCommands::Submit(commandPool, [&](VkCommandBuffer commandBuffer)
								   {
		VkBufferImageCopy region = {};
		...
		region.imageSubresource.layerCount = 1;
		region.imageOffset = { 0, 0, 0 };
		region.imageExtent = { extent_.width, extent_.height, extent_.depth };
