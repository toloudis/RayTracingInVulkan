...
namespace Assets
{
	...
	VolumeTextureImage::VolumeTextureImage(Vulkan::CommandPool &commandPool, const VolumeTexture &texture)
	{
		// Create a host staging buffer and copy the image into it.
		static constexpr int nChannels = 1;
		const VkDeviceSize imageSize = texture.Width() * texture.Height() * texture.Depth() * sizeof(uint16_t) * nChannels;
		const auto &device = commandPool.Device();
		...
		// Create the device side image, memory, view and sampler.
		image_.reset(new Vulkan::VolumeImage(
			device,
			VkExtent3D{
	...
	VolumeTextureImage::~VolumeTextureImage()
	{
		sampler_.reset();
		imageView_.reset();
		image_.reset();
