...
namespace Assets {
Model *Model::CreateRandomSphereGroup(int nSpheres, float groupRadius,
                                      float atomRadius, float atomRadiusMax,
  ...
  return Model::CreateSphereGroup(
      v, r, Material::Lambertian(vec3(0.7f, 0.7f, 0.7f)), true, name);
}
...
Model *Model::LoadModel(const std::string &filename,
  ...
  for (const auto &material : objReader.GetMaterials()) {
    ...
    m.Diffuse = vec4(material.diffuse[0], material.diffuse[1],
  ...
  // If the model did not specify normals, then create smooth normals that
  // conserve the same number of vertices. Using flat normals would mean
  // creating more vertices than we currently have, so for simplicity and better
  // visuals we don't do it. See
  // https://stackoverflow.com/questions/12139840/obj-file-averaging-normals.
  if (objAttrib.normals.empty()) {
    std::vector<vec3> normals(vertices.size());

    for (size_t i = 0; i < indices.size(); i += 3) {
      const auto normal =
          normalize(cross(vec3(vertices[indices[i + 1]].Position) -
                              vec3(vertices[indices[i]].Position),
                          vec3(vertices[indices[i + 2]].Position) -
                              vec3(vertices[indices[i]].Position)));
  ...
  std::cout << "(" << objAttrib.vertices.size() << " vertices, "
            << uniqueVertices.size() << " unique vertices, " << materials.size()
            << " materials) ";
  std::cout << elapsed << "s" << std::endl;

  return new Model(std::move(vertices), std::move(indices),
                   std::move(materials), nullptr, filename);
}
...
Model *Model::CreateCornellBox(const float scale) {
  ...
  return new Model(std::move(vertices), std::move(indices),
                   std::move(materials), nullptr, "CornellBox");
}
...
Model *Model::CreateBox(const vec3 &p0, const vec3 &p1,
  ...
  return new Model(std::move(vertices), std::move(indices),
                   std::vector<Material>{material}, nullptr, "box");
}

static void makeSphereMesh(std::vector<Vertex> &vertices,
                           std::vector<uint32_t> &indices, glm::vec3 center,
                           float radius, int slices = 32, int stacks = 16) {
  ...
  for (int j = 0; j <= stacks; ++j) {
    ...
    for (int i = 0; i <= slices; ++i) {
      ...
      const vec3 position(center.x + v * std::sin(i0), center.y + z,
                          center.z + v * std::cos(i0));

      const vec3 normal(n0 * std::sin(i0), n1, n0 * std::cos(i0));

      const vec2 texCoord(static_cast<float>(i) / slices,
...
Model *Model::CreateSphere(const vec3 &center, float radius,
                           const Material &material, const bool isProcedural,
                           const std::string &name) {
  const int slices = 32;
  const int stacks = 16;
  ...
  return new Model(
      std::move(vertices), std::move(indices), std::vector<Material>{material},
      isProcedural ? new SphereGroup(centers, radii) : nullptr, name);
}

Model *Model::CreateSphereGroup(const std::vector<glm::vec3> &center,
                                const std::vector<float> &radius,
                                const Material &material, bool isProcedural,
                                const std::string &name) {
  // little tiny spheres for mesh geometry.
  // this is still too much and we should instance individual spheres or do the
  // billboarded POINTS version
  const int slices = 2;
  const int stacks = 2;
...
Model *Model::CreateVolume(const vec3 &p0, const vec3 &p1,
                        const Material &material) {
  std::vector<Vertex> vertices = {
      Vertex{vec3(p0.x, p0.y, p0.z), vec3(-1, 0, 0), vec2(0), 0},
      Vertex{vec3(p0.x, p0.y, p1.z), vec3(-1, 0, 0), vec2(0), 0},
  ...
  return new Model(std::move(vertices), std::move(indices),
                   std::vector<Material>{material}, nullptr, "volume");
}
...
Model::Model(std::vector<Vertex> &&vertices, std::vector<uint32_t> &&indices,
             std::vector<Material> &&materials,
             const class Procedural *procedural, const std::string &name)
    : name_(name), vertices_(std::move(vertices)), indices_(std::move(indices)),
      materials_(std::move(materials)), procedural_(procedural) {}
