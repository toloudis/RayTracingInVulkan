...
size_t Model::number_of_chains() const { return chains_.size(); }
...
void Model::new_chain(const Protein &p) { chains_.push_back(p); }
...
void Model::process_line(const char *line) {
  ...
  if (lt == dsrpdb_internal::ATOM) {
    ...
    [[maybe_unused]] int numscan = sscanf(
        line, dsrpdb_internal::atom_line_iformat_,
