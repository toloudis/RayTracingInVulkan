...
parseOperatorList(const std::string &value) {
  // (a)
  // (1-5)
  // (1-5)(1-5)
  //
  ...
  // std::regex reg("\\(? ([^ \\(\\)] + )\\) ? ] *",
  // std::regex_constants::ECMAScript); const oeRegex = / \(? ([^ \(\)] + )\) ?
  // ] * / g;
  std::regex oeRegex("\\(?([^\\(\\)]+)\\)?", std::regex_constants::ECMAScript);
  std::smatch match;
...
struct ExtractCIF : readcif::CIFFile {
  ExtractCIF() {
    ...
    register_category("atom_site", [this]() { parse_atom_site(); });
    register_category("pdbx_struct_oper_list", [this]() { parse_transform(); });
    register_category("pdbx_struct_assembly_gen",
                      [this]() { parse_assemblies(); });
  }
  ...
  void parse_transform() {
    ...
    pv.emplace_back(get_column("vector[1]", Required), [&m](const char *start) {
    ...
    pv.emplace_back(get_column("vector[2]", Required), [&m](const char *start) {
    ...
    pv.emplace_back(get_column("vector[3]", Required), [&m](const char *start) {
  ...
  void parse_atom_site() {
    ...
    pv.emplace_back(get_column("type_symbol", Required),
                    [&atom](const char *start) { atom.element = *start; });
...
void LoadCIFAsScene(const std::string &filename,
  ...
  if (nEntities > 0) {
    ...
    for (auto e : extract.chains) {
      ...
      auto model = Model::CreateSphereGroup(
          std::move(vertices), std::move(radii),
          Material::Lambertian(randomInBox(1.0, 1.0, 1.0) +
                               glm::vec3(0.5, 0.5, 0.5)),
          true, filename + " :: " + e.first);
          ...
          if (iter != modelMap.end()) {
            ...
            for (auto &op : opsList) {
              ...
              if (xformiter == extract.transforms.end()) {
                ...
                //  todo: implement clipping post-load time for more dynamic
                //  updates elsewhere in code
                //  if (xform[2][3] > 0) {
                modelInstances.push_back(
                    ModelInstance(model, xformiter->second));
                //}
              }
            }
          }
    ...
    models.push_back(std::unique_ptr<Model>(Model::CreateSphereGroup(
        std::move(vertices), std::move(radii),
        Material::Lambertian(randomInBox(1.0, 1.0, 1.0) +
                             glm::vec3(0.5, 0.5, 0.5)),
        true, filename)));
...
Model *LoadCIF(const std::string &filename, const Material &material,
  ...
  return Model::CreateSphereGroup(std::move(vertices), std::move(radii),
                                  material, true, filename);
}

Model *LoadPDB(const std::string &filename, const Material &material,
               float scale) {
  std::cout << "- loading '" << filename << "'... " << std::flush;
  const auto timer = std::chrono::high_resolution_clock::now();
  const std::string materialPath =
  ...
  try {
    ...
    for (auto i = pdb.models_begin(); i != pdb.models_end(); ++i) {
      auto model = *i;
      for (size_t j = 0; j != model.number_of_chains(); ++j) {
        ...
        for (auto k = chain.atoms_begin(); k != chain.atoms_end(); ++k) {
          ...
          vertices.push_back(
              glm::vec3(p.x() * scale, p.y() * scale, p.z() * scale));
          radii.push_back(0.25f);
        }
#if 0
