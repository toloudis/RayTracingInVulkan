...
//{
	...
	const float isScattered = dot(direction, normal) < 0 ? 1.0 : 0.0;
	...
	const vec4 colorAndDistance = vec4(1.0,0.0,0.0, -1.0);//t);
	const vec4 scatter = vec4(normal + RandomInUnitSphere(Ray.RandomSeed), isScattered);
